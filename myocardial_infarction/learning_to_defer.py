...
def encode_documents(documents: list, tokenizer_path, max_input_length=512):
    """
...
class DocumentBertLSTM(nn.Module):
    ...
    def __init__(self, model_path):
        ...
        # mothod 1: from Andriy Mulyar's bert_document_classification github repository
        self.classifier = nn.Sequential(
            nn.Dropout(p=self.model_config.hidden_dropout_prob),
            nn.Linear(self.model_config.hidden_size, args['n_classes']+1),
            nn.Sigmoid()
        )
...
def calculate_scores(y_pred, y_true, args):
    df = pd.DataFrame(
        {'true_y': y_true,
         'pred_y': y_pred
         })
    df_1 = df[df.pred_y != args['n_classes']]
    df_2 = df[df.pred_y == args['n_classes']]
    ...
    with torch.set_grad_enabled(False):
        ...
        for i in list(range(args['test_chunks'])):
            ...
            for _, test_sample in enumerate(test_dataloader):
                ...
                test_preds = np.concatenate([test_preds, torch.max(out_mini.cpu().data, 1)[1].numpy().astype(int)])
                test_probs = np.concatenate((test_probs, out_mini.cpu().data.numpy()[:, 1]), axis=0)
        ...
        return test_probs.tolist(), test_preds.tolist(), test_labels
...
def defer_CrossEntropyLoss(outputs, labels, n_classes, alpha):
...
def train(df_train, df_test, args, seed_i, alpha):
    ...
    for i in list(range(args['train_chunks'])):
        ...
        for epoch in range(args['max_epoch']):
            ...
            for batch_ndx, sample in enumerate(train_dataloader):
                ...
                loss = defer_CrossEntropyLoss(batch_confidences, batch_correct_output, args['n_classes'], alpha)
    ...
    y_prob, y_pred, y_true = predict(model, df_test, args, seed_i)
    score_list = calculate_scores(y_pred, y_true, args)

    return score_list, y_prob, y_pred, y_true


if __name__ == "__main__":
    import argparse

    parser = argparse.ArgumentParser(description='Raw data dir and alpha')
    ...
    # Initiate model loss and expert loss function
    loss_model = nn.CrossEntropyLoss().to(args['device'])
    loss_expert = nn.CrossEntropyLoss().to(args['device'])

    alpha_tune = np.arange(1.07, 1.4, 0.01).tolist()
    alpha = alpha_tune[rid]
    ...
    # use the first seed from generated random seed list, in order to compare with ens0
    random.seed(args['seed'])
    list_seed = random.sample(range(100), args['n_ensembles'])
    seed_i = list_seed[0]
    score_list, y_prob, y_pred, y_true = train(df_train, df_test, args, seed_i, alpha)
    df_score.loc[0] = [alpha] + score_list

    df_score.to_csv('outputs/score_of_learning_to_defer_{}.csv'.format(alpha), index=False)
